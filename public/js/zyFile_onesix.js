...
var ZYFILE = {
		...
		funUploadFile : function(files){
			var self = this;
			if(!groupid&&(!self.funchecknames()||!$('#city').val()||!$('#bussiness').val()||!$('#community').val()||!$('#building').val()||!$('#room').val())){
				alert('请完成资料填写');
				return;
			}

			var file = files[self.hadUpload];
			if((!groupid)&&self.groupid==0){
				var data = {
				...
				var data = {
					name: $('#name_'+file.index).val(),
					groups_id: groupid?groupid:self.groupid,
					scenestyle: 6
				};
			}
			console.log(data);
