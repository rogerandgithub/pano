		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div class="status_bar" id="cityregion">';
		            html += '			    <div style="text-align: center;">'

		            if(!groupid){
			            html += '	                 <label>城区：</label><input onclick="SelCity(this,event);" class="name_input house_info" style="width:20%" id="city">'
			            html += '	                 <label>商圈：</label><input class="name_input house_info" id="bussiness">'
			            html += '	                 <label>小区：</label><input class="name_input house_info" id="community">'
			            html += '	                 <label>栋数：</label><input class="name_input room_info" id="building">'
			            html += '	                 <label>房号：</label><input class="name_input room_info" id="room">'
			        }
		            html += '			   </div>';
		            html += '			</div>';
				    ...
				    html += '							<img class="upload_image" src="add_img.png" style="width:30px" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
	            ...
	            $(self).append(html).css({"height":para.height});
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		</div>';
					html += '	</div>';
					html += '	<a style="padding: 5px;" href="javascript:;" class="imgBox">';
					html += '		<div class="uploadImg" style="width:250px">';				
					html += '			<canvas id="myCanvas'+file.index+'"></canvas>';
					html += '		</div>';
					html += '       <input id="name_'+file.index+'" placeholder="请输入名称" style="width:50%" class="name_input"/>';
					html += '	</a>';
			...
			this.render = function (src, i){
                var image = new Image();
                //var oMask = $('#mask');
                image.crossOrigin = "*";
                image.onload = function(){
                	if(image.height/image.width!=6&&image.width/image.height!=2){
	            		alert('你上传的不符合规格，只能上传一比二或者一比六的全景图');
            			return;
                	}
                	if(image.width/image.height==2){
	            		alert('你上传的是1:2格式的图片，将为你跳转到1:2接口上传');
            			location.href="/upload12?groupkey="+groupkey;
            			return;
                	}
						...
						var funDealtPreviewHtml = function() {
							...
							if (file) {
								...
								reader.onload = function(e) {
									...
									filePath.push(e.target.result);
									i++;
									// 再接着调用此方法递归组成可以预览的html
									funDealtPreviewHtml();
									funAppendPreviewHtml(html);

									self.render(e.target.result, i-1+self.pindex);
