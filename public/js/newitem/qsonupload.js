		...
		return this.each(function(){
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0){
					...
					html += '	<div class="loader"><div class="loader-inner square-spin"><div></div></div><p>图片处理中...</p></div>';
			...
			this.render = function (src, i, html, func){
                ...
                image.onload = function(){
                	...
                	if((image.height/image.width)==6)window.hadsix = true;
                };
				image.src = src;
            };
            this.pindex = 0;
...
var ZYFILE = {
		...
		funUploadFile : function(files){
			var self = this;
			if(!groupid){
				if(!self.funchecknames()||!$('#city').html()||!$('#bussiness').val()||!$('#community').val()||!$('#building').val()||!$('#room').val()||!$('#price').val()){
			...
			// tianjia userid
			data.userid = upload.userid;
			if(!window.highupload){
	            $.post('/tokenitem/upload12', data, function(res){
	                ...
	                Q.addEvent("progress", function(p, s) {
						var eleProgress = $("#uploadProgress_" + file.index), percent = p + '%';
						if(p==100){
							$('.loader').eq(self.hadUpload).show();
						}
						if(eleProgress.is(":hidden")){
							eleProgress.show();
						}
	                ...
	                Q.addEvent("putFinished", function(fsize, res, taking) {
	                    $("#uploadProgress_" + file.index).hide();
	                    $("#uploadSuccess_" + file.index).show();
						$('.loader').eq(self.hadUpload).html('上传成功').css('line-height','177px');
                ...
                for(var i in data){
                	console.log(i);
                	console.log(data[i]);
                	Q.AddParams(i,data[i]);
                }
            	Q.AddParams('uuid',uuid);
                Q.Upload(file);
			}
		},
		...
		init : function(e,type){  // 初始化方法，在此给选择、上传按钮绑定事件
			...
			if(self.uploadInput){
				...
				this.uploadInput.on("click", function(e) {
					window.highupload = $(this).attr('highupload')=='true';
					if(window.highupload&&window.hadsix){
						toast.showmsg('一比六图片不支持高级上传，请选择快速上传或者重新添加');
						return;
					}
