...
var upload_flg = 0;  // 控制上传
var limit_upload_num = 3;  // 上传数量控制器
// 每次只上传一套，用普通的方式上传
function startUpload(){
    console.log('sqing--------------------------');
    // 上传控制器判断
    ...
    db.Missions.findAll({
        ...
        queue.allArray(missions, sqing_convert, {'event_succ': console.log}).then(function(key){
            queueup();
        });
        ...
        function sqing_convert(obj){
            ...
            exec(cmd, function(err, stdout, stderr){
                if(err){
                    limit_upload_num--;  //如果拼接不成功，任务量也减一
                    return;
                };
                limit_upload_num--;  //如果拼接成功，任务量减一

                if(limit_upload_num == 0){
                    limit_upload_num = 3  // 全部拼接成功，将状态值恢复初始化
                    upload_flg = 0;  // 更改上传控制器
                }
                console.log('upload_flg:', upload_flg);
