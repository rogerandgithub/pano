...
function queueup(){
    ...
    utils.client.get('upNum', function(err, value){
        ...
        db.Missions.findAll({
            ...
            if(toLen > 10){
                missions.splice(10 - value, toLen - 10);
            }
...
function convert(obj){
    ...
    exec(cmd, function(err, stdout,stderr){
        console.log('---------------ssssssssssssssssssss-------------', stdout)
        if(err){
            //如果拼接不成功，任务量也减一
            subValue();
            db.Missions.update({
...
function uptoken(bucket, key) {
    console.log('uptokenuptokenuptokenuptokenuptokenuptokenuptokenuptokenuptokenuptokenuptoken')
    var putPolicy = new qiniu.rs.PutPolicy(bucket + ":" + key);
    console.log('1111nodeupcbnodeupcbnodeupcb')
    putPolicy.callbackUrl = 'http://120.76.27.228/tokenitem/nodeupcb';
    console.log('2222nodeupcbnodeupcbnodeupcb')
    putPolicy.callbackBody = 'filename=$(fname)&filesize=$(fsize)';
    console.log('3333nodeupcbnodeupcbnodeupcb')
